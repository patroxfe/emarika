...
export default function Nav() {
  ...
  useEffect(() => {
    if (location.pathname.includes('brief') || location.pathname.includes('regulamin') || location.pathname.includes('polityka')) {
      setIsDarkNav(true);
    } else {
      setIsDarkNav(false);
    }
  ...
  const isBriefOrTermsOrPrivacy =
    location.pathname === '/emarika/brief' ||
    location.pathname === '/emarika/regulamin/' ||
    location.pathname === '/emarika/polityka/';
  ...
  return (
    ...
    >
      <div className='wrapper flex justify-between items-center'>
        {isBriefOrTermsOrPrivacy ? (
          <Link to='/emarika/' className='text-xl font-semibold z-50 cursor-pointer'>
            eMarika.pl
          </Link>
        ) : (
        ...
        <NavLinks isOpen={isOpen} setIsOpen={setIsOpen} scrollToSection={scrollToSection} isBriefOrTermsOrPrivacy={isBriefOrTermsOrPrivacy} />
